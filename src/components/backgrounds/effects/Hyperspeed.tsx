// Hyperspeed — React-Bits-Komponente: WebGL-Highway aus rasenden Car-Lights
// und neon-farbenen Light-Sticks am Strassenrand. Three.js + postprocessing
// (Bloom + SMAA). Click hold = beschleunigt (FOV-Punch).
//
// Schwerer als unsere CSS-Backgrounds — ist deshalb lazy-loaded und nur
// gemounted wenn der User ihn aktiv waehlt. Dispose-Logic raeumt WebGL-
// Context + Event-Listener beim Unmount auf.
//
// Adapted from reactbits.dev/Hyperspeed (JS + CSS variant).
...
const DEFAULT_EFFECT_OPTIONS: Required<Omit<EffectOptions, 'distortion'>> & { distortion: string } = {
  onSpeedUp: () => {},
  onSlowDown: () => {},
...
export function Hyperspeed({ preview = false }: { preview?: boolean }) {
  const hyperspeed = useRef<HTMLDivElement>(null);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const appRef = useRef<any>(null);

  useEffect(() => {
    const effectOptions: EffectOptions = preview
      ? { ...DEFAULT_EFFECT_OPTIONS, lightPairsPerRoadWay: 20, totalSideLightSticks: 12 }
    ...
    const distortions: Record<string, { uniforms: Record<string, { value: unknown }>; getDistortion: string; getJS?: (progress: number, time: number) => THREE.Vector3 }> = {
      mountainDistortion: {
    ...
    const pickRandom = <T,>(arr: T | T[]): T => (Array.isArray(arr) ? arr[Math.floor(Math.random() * arr.length)] : arr);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    class Road {
      webgl: any; options: any; uTime = { value: 0 };
    ...
    const roadMarkings_vars = `
      ...
      highp float random(vec2 co) { highp float a = 12.9898; highp float b = 78.233; highp float c = 43758.5453;
        highp float dt = dot(co.xy, vec2(a, b)); highp float sn = mod(dt, 3.14); return fract(sin(sn) * c); }`;
    ...
    class App {
      ...
      renderer: THREE.WebGLRenderer; composer: EffectComposer; camera: THREE.PerspectiveCamera; scene: THREE.Scene;
      fogUniforms: Record<string, { value: unknown }>; clock: THREE.Clock; assets: { smaa?: { search?: HTMLImageElement; area?: HTMLImageElement } }; disposed: boolean;
      ...
      tick() {
        ...
        if (!this.hasValidSize) {
          ...
          if (w > 0 && h > 0) {
            ...
            requestAnimationFrame(this.tick); return;
          }
        }
  ...
  return <div className={`bg-hyperspeed ${preview ? 'is-preview' : ''}`} ref={hyperspeed} />;
}
