  ...
  return (
    <div className="k-col">
      <div className="k-col-head">
        ...
        <button className="add" onClick={() => setAdding(true)} title="Aufgabe hinzufügen">
      ...
      >
        ...
        {!adding && tasks.length === 0 && !dragOver && (
          <button className="k-add-btn" onClick={() => setAdding(true)}>
            <Icon name="plus" size={12} /> Aufgabe hinzufügen
          </button>
        )}
      </div>
    </div>
...
export function BoardKanban({ tasks }: BoardKanbanProps) {
  ...
  const onDrop = (col: ColumnId) => {
    ...
    if (dragTask.col !== col) {
      ...
      showToast(`Verschoben → ${colLabel}`);
    }
    setDragTask(null);
    setDragOverCol(null);
  };
  const onTaskClick = (t: Task) => setUI({ taskDetailId: t.id });
